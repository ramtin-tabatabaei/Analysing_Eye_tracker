...
#     if Target == "all":
#         return (len(seq)-2*numebr_of_puzzles+1)/numebr_of_puzzles
#     else:
...
# def process_sessions(base_dir, output_csv):
...
#             # select rows for this puzzle, before participant turn
#             if loop == 0:
#                 sub = df[
#                             (df["Puzzle_number"] == puzzle) &
#                             (df["failure_number"] == "No failure")
#                         ]
#             elif loop == 1:
#                 sub = df[
#                             (df["Puzzle_number"] == puzzle) &
...
#             numebr_of_puzzles = len(list(set(sub["object_number"])))
#             # print(float((sub["timestamp"].iloc[-1]-sub["timestamp"].iloc[0]))/numebr_of_puzzles)
...
#                 "num_gaze_shifts": gaze_shift_counter(seq, numebr_of_puzzles, "all"),
...
#                 "transition_entropy": transition_entropy(P),
#                 "stationary_entropy": stationary_entropy(P),
#                 "matrix_norm": np.linalg.norm(P, ord='fro')
#             }
#             records.append(rec)
...
#             continue
#         input_file = f'/Volumes/R@mtin/Year 2/Experiment/EyeTracker/Data/Participant {i}/merged_gaze_state.csv'
#         output_file = f'/Volumes/R@mtin/Year 2/Experiment/EyeTracker/Data/Participant {i}/participant_gaze_summary.csv'

#         process_sessions(input_file, output_file)
